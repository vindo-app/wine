...
static BOOL getstring_test(LPCWSTR assocName, HKEY progIdKey, ASSOCSTR str, LPCWSTR expected_string) {
    ...
    if (hr != S_FALSE) {
        if (expected_string) {
            ok(SUCCEEDED(hr), "GetString returned 0x%x, expected success\n", hr);
            if (FAILED(hr)) return FALSE;
        } else {
            ok(FAILED(hr), "GetString returned 0x%x, expected failure\n", hr);
    ...
    if (expected_string) {
        ...
        if (lstrcmpW(buffer, expected_string) != 0) return FALSE;
    }
    return TRUE;
}
...
#define TEST_GET_STRING(a,p,s,es) \
    ...
    TEST_GET_STRING(test_extensionW, NULL, ASSOCSTR_DEFAULTICON, NULL);
    TEST_GET_STRING(test_progidW, NULL, ASSOCSTR_DEFAULTICON, NULL);
    TEST_GET_STRING(NULL, test_progid_key, ASSOCSTR_DEFAULTICON, NULL);
    ...
    TEST_GET_STRING(test_extensionW, NULL, ASSOCSTR_DEFAULTICON, test_iconW);
    TEST_GET_STRING(test_progidW, NULL, ASSOCSTR_DEFAULTICON, test_iconW);
    TEST_GET_STRING(NULL, test_progid_key, ASSOCSTR_DEFAULTICON, test_iconW);
